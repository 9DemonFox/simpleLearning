...
def update(trainX,trainY,epoch,n,N,m,D,q,u,σ2,clf,y0,z,M):
    ...
    for i in range(epoch):
        ...
        c = 0
        for t in range(n):
            f = stats.multivariate_normal.pdf((trainY[t*m:(t+1)*m,0:1]).reshape(m), mean=(y1[t*m:(t+1)*m,0:1] + np.dot(z[t],u[t])).reshape(m), cov=σ2*np.identity(m))
            c = c + stats.multivariate_normal.pdf((trainY[t*m:(t+1)*m,0:1]).reshape(m), mean=(y1[t*m:(t+1)*m,0:1] + np.dot(z[t],u[t])).reshape(m), cov=σ2*np.identity(m))
            l[t] = f
        ...
        for j in range(n):
            ...
            B = Q*np.identity(q)
            I = M*math.pow(σ2*2*math.pi,m/(-2))*np.power(A,1/2)*np.power(B,1/2)*math.exp((np.dot(np.dot((trainY[j*m:(j+1)*m,0:1] - y1[j*m:(j+1)*m,0:1]).T,U),trainY[j*m:(j+1)*m,0:1] - y1[j*m:(j+1)*m,0:1]))/(2*σ2))
            ...
            if a == 1:
                d = np.identity(q)
                mean = np.dot(np.dot(np.linalg.inv(np.dot(z[j].T,z[j]) + σ2*np.divide(d,D, out=np.zeros_like(d), where= D !=0)),z[j].T),trainY[j*m:(j+1)*m,0:1] - y1[j*m:(j+1)*m,0:1])                
                d = np.identity(q)
                cov = σ2*np.linalg.inv(np.dot(z[j].T,z[j]) + σ2*np.divide(d,D, out=np.zeros_like(d), where= D !=0))
